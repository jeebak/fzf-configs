...
- `utils.sh` — shared utilities: `fzf-base()` (common fzf wrapper adding `--ansi --border --border-label` and `--tmux "WxH"` when in tmux, width/height from `#{client_width}`/`#{client_height}` at 90% capped at 180×60); `qt`, `qte`, `reo` (output redirection helpers); `cmd_exists`, `errcho`, `die`, `clip` (clipboard via OSC52 + native fallbacks)
- `git/commands.sh` — sources `lib/utils.sh`; defines all git functions: `gf` (files/status), `gb` (branches), `gt` (tags), `gh` (hashes/log), `gr` (remotes), `ga` (aliases), `gl` (log browser), `gs` (stashes), `gw` (worktrees), `grl` (reflog), `edit-modified`, `g?` (interactive `git help -a` browser); exports all functions to subshells via `eval "$(declare -F | sed -e 's/-f /-fx /')"`
...
**TMUX-aware popup:** `fzf-base()` (in `lib/utils.sh`) adds `--tmux "WxH"` when `$TMUX` is set, opening all fzf browsers as tmux popups (inspired by junegunn/fzf-git.sh). Width and height are computed from `#{client_width}`/`#{client_height}` at 90%, capped at 180 cols × 60 rows. The `gf()` ctrl-u/ctrl-e/ctrl-o/ctrl-p bindings are always available regardless of tmux.
